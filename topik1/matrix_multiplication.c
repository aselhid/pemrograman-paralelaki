...
main(int argc, char **argv)
{
  ...
  if (taskid == 0)
  {
    ...
    for (i = 1; i <= numworkers; i++)
    {
      ...
      MPI_Recv(&tempfloat, 1, MPI_FLOAT, source, 2, MPI_COMM_WORLD, &status);
      printf("%.6f\n",workersendfloat);
      workersendfloat = (workersendfloat > tempfloat) ? tempfloat : workersendfloat;
    }
    ...
    printf("%.6f,", (finishjob.tv_sec + finishjob.tv_usec * 1e-6) - workersendfloat);
    printf("\n");
  }
  ...
  if (taskid > 0)
  {
    ...
    gettimeofday(&workersendtime, 0);
    workersendfloat = workersendtime.tv_sec + workersendtime.tv_usec * 1e-6;

    MPI_Send(&offset, 1, MPI_INT, 0, 2, MPI_COMM_WORLD);
    MPI_Send(&rows, 1, MPI_INT, 0, 2, MPI_COMM_WORLD);
    MPI_Send(&c, rows * N, MPI_DOUBLE, 0, 2, MPI_COMM_WORLD);
    MPI_Send(&workersendfloat, 1, MPI_FLOAT, 0, 2, MPI_COMM_WORLD);
  }
